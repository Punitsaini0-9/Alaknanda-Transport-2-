(function($) {
    ...
    if ($("body").hasClass("tt-transition")) {
        ...
        function RevealLoad() {
            ...
            tl_transitIn.set("#page-transition", {
                autoAlpha: 1
            });
            tl_transitIn.to(
                ".ptr-overlay", {
            ...
            tl_transitIn.to("#content-wrap", {
                y: -80,
                autoAlpha: 0
            }, 0);
            tl_transitIn.to("#tt-header", {
                y: -20,
                autoAlpha: 0
            }, 0);
            tl_transitIn.to(".ptr-preloader", {
                autoAlpha: 1
            }, 0.4);
        }
        ...
        function HideLoad() {
            ...
            // duration: 1.5, change by pradeep
            ...
            // duration: 1.5, stagger 0.15, changes by pradeep
    ...
    if ($("body").not(".is-mobile").hasClass("tt-magic-cursor")) {
        if ($(window).width() > 1024) {
            ...
            var $ballWidth = 40; // Ball default width
            var $ballHeight = 40; // Ball default height
            var $ballScale = 1; // Ball default scale
            var $ballOpacity = 0.7; // Ball default opacity
            ...
            function initCursorEffect() {
                $("[data-cursor]").each(function() {
                    $(this)
                        .off("mouseenter mouseleave") // Remove any previous handlers to avoid duplicate bindings
                        .on("mouseenter", function() {
            ...
            // Page nav hover.
            $(".awards-section-box").each(function() {
                if ($(this).find(".tt-pn-image").length) {
                    $(this)
                        .on("mouseenter mouseover", function() {
                            console.log("hovered me");
                            $("#magic-cursor").addClass("tt-pn-hover-on");
                            $(this).find(".tt-pn-image").appendTo($ball);
                            gsap.to($ball, {
                                duration: 0.3,
                                width: "10vw",
                                height: "10vw",
    ...
    $olMenuToggleBtn.on("click", function() {
        ...
        if ($("body").hasClass("tt-ol-menu-open")) {
            ...
            tl_olMenuIn.to(".tt-overlay-menu", {
                duration: 0.4,
                autoAlpha: 1
            });
            ...
            $(".tt-overlay-menu a, .tt-logo a")
                ...
                .not('[href^="tel"]') // omit from selection
                .on("click", function() {
                    gsap.set("#content-wrap, .ttgr-cat-nav", {
                        autoAlpha: 0
                    }); // Hide before timeline
            ...
            tl_olMenuOut.set(".tt-ol-menu-list > li", {
                clearProps: "all"
            }); // clearProps only
    ...
    if ($(".tt-search").length) {
        ...
        $(".tt-search-close, .tt-search-close-btn").on("click", function() {
            ...
            tl_ttSearchOut.to(".tt-search-appear", {
                duration: 0.5,
                y: -80,
                autoAlpha: 0,
                stagger: 0.05,
        ...
        $(".tt-portfolio-slider").each(function() {
            ...
            if (!isMobile) {
                if ($ttPortfolioSlider.data("parallax-mouse-move")) {
                    gsap.set($ttPortfolioSlider.find(".tt-psi-image"), {
                        scale: 1.05
                    });
    ...
    if ($(".ph-share").length) {
        ...
        $(".ph-share-content").prepend(
            '<div class="ph-share-close-btn"><i class="fas fa-times"></i></div>'
        );
    ...
    if ($("#page-header").hasClass("ph-content-parallax")) {
        ...
        if ($(".ph-caption-subtitle").length) {
            ...
            tlPhParallax.to(".ph-subt-parallax", {
                y: -10
            }, 0);
        }
        if ($(".ph-caption-title-ghost").length) {
            ...
            tlPhParallax.to(".ph-ghost-parallax", {
                y: 40
            }, 0);
        }
    ...
    $(".ttgr-cat-trigger").on("click", function() {
        ...
        if ($("body").hasClass("ttgr-cat-nav-open")) {
            gsap.to(".portfolio-grid-item", {
                duration: 0.3,
                scale: 0.9
            });
            gsap.to(".pgi-caption, .ttgr-cat-trigger", {
                duration: 0.3,
                autoAlpha: 0,
            });
    ...
    // Career page tabs
    $(".career-tab").click(function() {
        $(".job-opening-box").removeClass("active");
        var activeMenuT = $(this).attr("data-content-id");
        $("." + activeMenuT).addClass("active");

        // Tabs Active
        $(".career-tab").removeClass("active");
        $(this).addClass("active");
    });

    // About Us page location map
    $(".map-section .footer-address-box").on("click mouseover", function() {
        $(".location-pin").removeClass("active");
        var activeAddress = $(this).attr("data-address");
        $("." + activeAddress).addClass("active");

        // Tabs Active
        $(".map-section .footer-address-box").removeClass("active");
        $(this).addClass("active");
    });

    $(".location-pin").on("click mouseover", function() {
        $(".map-section .footer-address-box").removeClass("active");
        var activeLocation = $(this).attr("data-location");
    ...
    if ($(".tt-sidebar").length) {
        ...
        // Wrap sidebar.
        $(".tt-sidebar").wrap(
            '<div class="tt-sidebar-holder"><div class="tt-sidebar-wrap"></div></div>'
        );

        // Append sidebar cover and trigger
        $(".tt-sidebar-wrap").append(
            '<div class="tt-sidebar-cover"></div> <div class="tt-sidebar-trigger"><span class="tt-str-icon"><i class="fas fa-align-justify"></i></span><span class="tt-str-icon-close"><i class="fas fa-times"></i></span></div>'
        );

        // Prepend sidebar top close button.
        $(".tt-sidebar").prepend(
            '<div class="tt-sidebar-close-btn"><i class="fas fa-times"></i></div>'
        );
    ...
    if ($(".tt-sliding-sidebar-wrap").length) {
        ...
        // Prepend top close button.
        $(".tt-sliding-sidebar").prepend(
            '<div class="tt-sliding-sidebar-close-btn"><i class="fas fa-times"></i></div>'
        );
        ...
        if ($(".tt-header-tools-dynamic").length) {
            function ttMoveSlidingSidebarTriggerToHtd() {
                if (window.matchMedia("(max-width: 768px)").matches) {
                    if ($(".to-shop-list-btn-wrap").length) {
                        $(".tt-sliding-sidebar-trigger").insertAfter(
                            ".to-shop-list-btn-wrap"
                        );
                    } else {
                        $(".tt-sliding-sidebar-trigger").prependTo(
        ...
        $(".tt-sliding-sidebar-close, .tt-sliding-sidebar-close-btn").on(
            "click",
            function() {
    ...
    $(
        ...
        if ($("body").hasClass("tt-demo-btn-active")) {
            // Show menu background with delay and duration
            gsap.to(".tt-overlay-menu", {
                duration: 0.3, // Reduced duration
                autoAlpha: 1, // Animates both visibility and opacity
                delay: 0.2, // Matches the delay of tt-demo-btn
            });

            // Scale in and fade in the demo button
            gsap.to(".tt-demo-btn", {
            ...
            // Hide menu background with animation
            gsap.to(".tt-overlay-menu", {
                duration: 0.2, // Reduced duration
                autoAlpha: 0, // Animates both visibility and opacity
            });

            // Scale out and fade out the demo button
            gsap.to(".tt-demo-btn", {
                duration: 0.2, // Reduced duration
                scale: 0.9,
                autoAlpha: 0,
    ...
    if ($(".tt-content-carousel-slider").length) {
        $(".tt-content-carousel-slider").each(function() {
            var $ttContentCarousel = $(this);
                    ...
                    $ttContentCarousel
                        ...
                        .on("mousedown touchstart pointerdown", function(e) {
                            if (e.which === 1) {
                                // Affects the left mouse button only!
                                gsap.to(
                                    $ttContentCarousel.find(".tt-content-carousel-item-slide"), {
                                        duration: 0.7,
                                        scale: 0.9
                                    }
                                );
    ...
    var u = $(window).height();
    gsap.utils.toArray(".our-client-section").forEach((e) => {
        ScrollTrigger.create({
            trigger: e,
            start: "top center",
            end: "bottom center",
    ...
    $(window).on("load", function() {
        setTimeout(() => {
            ...
            if (window.matchMedia("(min-width: 1250px)").matches) {
                if ($(".section-video").length > 0) {
                    var videoSectionTimeline = gsap.timeline({
                        scrollTrigger: {
                            trigger: ".section-video",
                            start: "top top",
                            ...
                            anticipatePin: 1,
                            onEnter: () => {
                                $("#tt-header").addClass("redLogo");
                            },
                            onLeave: () => {
                                $("#tt-header").removeClass("redLogo");
                            },
                            onEnterBack: () => {
                                $("#tt-header").addClass("redLogo"); // Re-add the class when scrolling back
                            },
                            onLeaveBack: () => {
                                $("#tt-header").removeClass("redLogo");
                            },
                    ...
                    videoSectionTimeline
                        .set(".section-video .container-video", {
                            clipPath: "polygon(48% 45%, 53% 45%, 53% 52%, 48% 52%)",
                            backgroundColor: "#e60200",
                        })
                        .set(".section-video .container-video video", {
                            opacity: 0.2,
                        })
                        .from(".section-video", {
                            scale: 0.7,
                        ...
                        .from(".section-video .container-video", {
                            scale: 0.4,
                            duration: 0.1,
                        })
                        .to(".section-video .container-video", {
                            clipPath: "polygon(25% 10%, 73% 10%, 73% 90%, 25% 90%)",
                        })
                        .to(".section-video .container-video", {
                            clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)",
                        });

                    const videoUpTitleTimeline = gsap.timeline({
                        scrollTrigger: {
                            trigger: "body",
                            start: "top top",
                    ...
                    videoUpTitleTimeline.to(".section-video .titleUp", {
                        y: "0%"
                    }).to(
                        ".section-video .titleUp", {
                            opacity: 0,
                            y: "-400%",
                            duration: 0.6,
                        },
                ...
                aa.to(".section-video .container-video video", {
                    opacity: 1,
                    duration: 1,
                }).to(".section-video .container-video", {
                    backgroundColor: "transparent",
                ...
                const videoDownTitleTimeline = gsap.timeline({
                    scrollTrigger: {
                        trigger: "body",
                        start: "top top",
                        end: () => window.innerHeight * 1.3,
                ...
                videoDownTitleTimeline
                    .to(".section-video .titleDown", {
                        y: "0%"
                    })
                    .to(".section-video .titleDown", {
                        opacity: 0,
        ...
        // const pcateg = document.querySelector(".lifePinned");
        // const gallery = document.querySelector(".lifePinned .swiper-wrapper");
        // const pcategBox = document.querySelector(".lifePinned .swiper-slide");
        // const pcategBoxes = document.querySelectorAll(".lifePinned .swiper-slide");

        // if (pcateg) {
        // //   const galleryWidth =
        // //     pcategBox?.clientWidth * pcategBoxes.length - window.innerWidth * 1.2;
        ...
        const swiper = new Swiper('.swiper1', {
            ...
            simulateTouch: true, // Disable touch interactions
            allowTouchMove: true, // Fully disable drag/swipe
            navigation: {
                nextEl: '.tt-cc-nav-next', // Right arrow
                prevEl: '.tt-cc-nav-prev', // Left arrow
            },
            pagination: {
            ...
            lazy: {
                loadPrevNext: true, // Lazy load images for smoother performance
            },

            // Set different slidesPerView based on screen width
            breakpoints: {
                320: { // For devices with a screen width of 320px or more
                    slidesPerView: 1,
                    slidesPerView: 1.3,
                    stagePadding: 10,
                    centeredSlides: true,
                ...
                768: { // For devices with a screen width of 768px or more
                    slidesPerView: 2,
                    //  slidesPerView: 1,
                    // centeredSlides: true,
                    // slidesPerView: 2.3,
                ...
                1024: { // For devices with a screen width of 1024px or more
                    slidesPerView: 3,
                    //  slidesPerView: 1,
                    // centeredSlides: true,
                    // slidesPerView: 3.3,
                ...
                1440: { // For devices with a screen width of 1440px or more
        ...
        const videos = document.querySelectorAll('.swiper1 video');
        videos.forEach(video => {
            video.addEventListener('play', () => {
                swiper.allowTouchMove = false; // Disable swipe when video is playing
            });

            video.addEventListener('pause', () => {
                swiper.allowTouchMove = true; // Enable swipe when video is paused
            });

            video.addEventListener('ended', () => {
                swiper.allowTouchMove = true; // Enable swipe when video ends
            });
        });
    ...
    $(".revamped--work [class*='tt-col-']").on("mouseenter", function() {
        $(this).closest(".tt-row").addClass("hover");
    });
    $(".revamped--work [class*='tt-col-']").on("mouseleave", function() {
        $(this).closest(".tt-row").removeClass("hover");
    });

    var cardSwiperSec = new Swiper(".cardSwiperSec .swiper", {
        effect: "cards",
        grabCursor: true,
        speed: 700,

        navigation: {
            nextEl: ".cardSwiperSec__nextWrapper h5",
        },

        on: {
            slideChange: function() {
                var activeSlide = this.slides[this.activeIndex];
                var attrValue = activeSlide.getAttribute("data-id");
                $("#btb_show_id").val(attrValue);
            },
        },
    });

    var $workClientsSec__grid = $(".workClientsSec__grid");
    $workClientsSec__grid.imagesLoaded(function() {
        $workClientsSec__grid.isotope({
            itemSelector: ".workClientsSec__item",
            layoutMode: "packery",
            transitionDuration: "0.7s",
            percentPosition: true,
        });
        ...
        setTimeout(function() {
            $workClientsSec__grid.isotope("layout"); // Refresh Isotope
            ScrollTrigger.refresh(true); // Refresh ScrollTrigger
        }, 500);
    });

    // Filter
    /*$(".workClientsSec .filter-tab").on("click", function () {
      var selector = $(this).attr("data-filter");
      $workClientsSec__grid.isotope({
        filter: selector,
      });
    ...
    // Filter item active
    var filterItemActive = $(".workClientsSec .filter-tab");
    filterItemActive.on("click", function () {
      var $this = $(this);
      if (!$this.hasClass("active")) {
        filterItemActive.removeClass("active");
    ...
    if ($(".workTabBtn").hasClass("active")) {
        $(".workMainTabs .activeBar").css({
            width: $(".workMainTabs__btn.active").width(),
            left: 0,
        });
    } else {
        $(".workMainTabs .activeBar").css({
            width: $(".workMainTabs__btn.active").width(),
            left: $(".workMainTabs__btn:not(.active)").width() + 30,
        });
    }

    setTimeout(() => {
        $(".workClientsSec").fadeIn(function() {
            $workClientsSec__grid.isotope("layout");
        });
    }, 500);

    $(".workMainTabs__btn").on("click", function() {
        // ScrollTrigger.refresh(true);
        // if ($(this).hasClass("workTabBtn")) {
        //   $(".clientsTabBtn").removeClass("active");
        //   $(".workTabBtn").addClass("active");
        //   $(".workMainTabs .activeBar").css({
        //     width: $(".workTabBtn").width(),
        //     left: 0,
        //   });
        //   $("#portfolio-grid").fadeIn(function () {
        //     $(".workClientsSec").fadeOut();
        //   });
        // }
        // if ($(this).hasClass("clientsTabBtn")) {
        //   $(".workTabBtn").removeClass("active");
        //   $(".clientsTabBtn").addClass("active");
        //   $(".workMainTabs .activeBar").css({
        //     width: $(".clientsTabBtn").width(),
        //     left: $(".workTabBtn").width() + 30,
        //   });
        //   $("#portfolio-grid").fadeOut(function () {
        //     $(".workClientsSec").fadeIn(function () {
        //       $workClientsSec__grid.isotope("layout");
        //     });
        //   });
    ...
    $multiDrop = $(".lookingFor").select2({
        placeholder: "What are you looking for?",
        minimumResultsForSearch: Infinity,
    });

    if (document.querySelector(".newBlogSec .swiper")) {
        new Swiper(".newBlogSec .swiper", {
            slidesPerView: 1.2,
            spaceBetween: 15,
            freeMode: true,
            grabCursor: true,
    ...
    if (document.querySelector("[data-animation=type-writter]")) {
        const words = document
            .querySelector("[data-animation=type-writter]")
        ...
        words.forEach((word) => {
            e.to("[data-animation=type-writter]", {
                duration: 0.6,
                text: {
                    value: word,
                    type: "diff"
            ...
            e.to(
                "[data-animation=type-writter]", {
                    duration: 1,
                    ease: "none",
                    onStart: () => clearText("[data-animation=type-writter]"),
                },
                "+=1"
            );
        });
        ...
        function clearText(selector) {
            ...
            for (let i = textLength; i >= 0; i--) {
                ...
                }, (textLength - i) * (interval * 500));
            }
        }
    }

    var showInnerVideo = document.querySelector(".showInner__video video");

    function muteVideo() {
        showInnerVideo.muted = true;
    }

    function unMuteVideo() {
        showInnerVideo.muted = false;
    }
    ...
    if (muteBtn) {
        ...
        $(window).on("load", function() {
            setTimeout(() => {
                showInnerVideo.play();
                $(".showInner__video").addClass("playing");
            }, 2000);
        });
    }

    const showsvideo = document.querySelector("#showsvideo");
    ...
    if (showsvideo) {
        showsvideo.ontimeupdate = function() {
            var percentage = (showsvideo.currentTime / showsvideo.duration) * 100;
        ...
        $("#custom-seekbar").on("click", function(e) {
            ...
            var vidTime = showsvideo.duration * percentage;
            showsvideo.currentTime = vidTime;
        });
    }
    ...
    playButtons.forEach((button) => {
        const videoId = button.getAttribute("data-play");
    ...
    /*$(".cardSwiperSec__showmeWrapper h5, .cardSwiperSec__card").on(
      "click",
      function () {
        $("html").css("overflow", "hidden");
        setTimeout(() => {
          $(".showsDialog").css("overflow", "auto");
        }, 1100);
        $(".showsDialog").addClass("active").css({ opacity: 1 });
      }
    );*/

    $(".showsDialog").on("click", function(e) {
        if (!$(e.target).closest(".showsDialog__inner").length) {
    ...
    $(".showsDialog .close").on("click", function() {
        $("html").css("overflow", "unset");
        $(".showsDialog")
            .removeClass("active")
            .css({
    ...
    $(".jobFliter").on("click", function() {
        $("html").css("overflow", "hidden");
        $(".filterDialog").css("overflow", "auto");
        $(".filterDialog").addClass("active");
    });

    $(".filterDialog__closeBtn, .filterDialog__applyBtn").on(
        "click",
        function() {
            $("html").css("overflow", "unset");
            $(".filterDialog").removeClass("active").css({
                overflow: "unset"
            });
        }
    );

    $(".filterDialog__accordion__btn").on("click", function() {
        $(this)
            .closest(".filterDialog__accordion__item")
            .find(".outer")
            .slideToggle();
    ...
    const items = gsap.utils.toArray(".easter__item");
    ...
    $(".moreEpisodes__tabs > div").on("click", function() {
        $(".moreEpisodes__tabs > div").removeClass("active");
        $(this).addClass("active");
        if ($(this).hasClass("popular")) {
            $(".popular__content").fadeIn();
            $(".recent__content").fadeOut();
        } else {
            $(".popular__content").fadeOut();
            $(".recent__content").fadeIn();
        }
    });

    var swiper = new Swiper(".movingPhotos", {
        slidesPerView: 4,
        loop: true,
        speed: 5000,
        autoplay: {
    ...
    // imagesLoaded('.moving-photos-grid', function () {
    //     var swiper = new Swiper(".movingPhotos", {
    //         slidesPerView: 4,
    //         loop: true,
    //         speed: 5000,
    ...
    var swiper = new Swiper(".movingPhotosLTR", {
        slidesPerView: 4,
        loop: true,
        speed: 5000,
        autoplay: {
